...
open class MarqueeLabel: UILabel, CAAnimationDelegate {
    ...
    @IBInspectable open var scrollDuration: CGFloat {
        get {
            switch speed {
            case let .duration(duration): return duration
    ...
    @IBInspectable open var scrollRate: CGFloat {
        get {
            ...
            case let .rate(rate): return rate
            }
        }
        set {
            speed = .rate(newValue)
    ...
    open func labelShouldScroll() -> Bool {
        ...
        let labelTooLarge = (sublabelSize().width + leadingBuffer) > bounds.size.width + CGFloat.ulpOfOne
    ...
    private func applyGradientMask(_ fadeLength: CGFloat, animated: Bool, firstStep _: MarqueeStep? = nil) {
    ...
    fileprivate weak var maskLayer: CAGradientLayer? {
        return layer.mask as! CAGradientLayer?
    }
    ...
    @objc public func restartForViewController(_ notification: Notification) {
        if let controller = (notification as NSNotification).userInfo?["controller"] as? UIViewController {
            if controller === firstAvailableViewController() {
                restartLabel()
            }
        }
    }
    ...
    @objc public func labelWasTapped(_: UIGestureRecognizer) {
        if labelShouldScroll() && !awayFromHome {
            updateAndScroll()
        }
    }
     ...
     */
    open func labelReturnedToHome(_: Bool) {
        // Default implementation does nothing - override to customize
        return
    }
    ...
    open override var isAccessibilityElement: Bool {
        didSet {
            sublabel.isAccessibilityElement = isAccessibilityElement
        }
    }
