...
extension String {
    ...
    func unicodeScalar(_ i: Int) -> UnicodeScalar {
        return unicodeScalars.prefix(i + 1).last!
    }

    func string(_ offset: Int, _ count: Int) -> String {
        let truncStart = unicodeScalars.count - offset
        return String(unicodeScalars.suffix(truncStart).prefix(count))
    }
    ...
    func indexOf(_ substring: String, _ offset: Int) -> Int {
        ...
        let maxIndex = count - substring.count
        if maxIndex >= 0 {
            for index in offset ... maxIndex {
                let rangeSubstring = self.index(startIndex, offsetBy: index) ..< self.index(startIndex, offsetBy: index + substring.count)
                #if swift(>=4)
    ...
    func substring(_ beginIndex: Int) -> String {
        return String.split(self, beginIndex, count - beginIndex)
    }
    ...
    func replaceAll(of pattern: String, with replacement: String, options _: NSRegularExpression.Options = []) -> String {
        do {
