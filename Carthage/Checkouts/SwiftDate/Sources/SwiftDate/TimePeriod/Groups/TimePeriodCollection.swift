...
open class TimePeriodCollection: TimePeriodGroup {
    ...
    public func sort(by type: SortType) {
        switch type {
        case let .duration(mode): periods.sort(by: sortFuncDuration(mode))
        case let .start(mode): periods.sort(by: sortFunc(byStart: true, type: mode))
        case let .end(mode): periods.sort(by: sortFunc(byStart: false, type: mode))
        case let .custom(f): periods.sort(by: f)
        }
    }
    ...
    public func sorted(by type: SortType) -> TimePeriodCollection {
        ...
        switch type {
        case let .duration(mode): sortedList = periods.sorted(by: sortFuncDuration(mode))
        case let .start(mode): sortedList = periods.sorted(by: sortFunc(byStart: true, type: mode))
        case let .end(mode): sortedList = periods.sorted(by: sortFunc(byStart: false, type: mode))
        case let .custom(f): sortedList = periods.sorted(by: f)
        }
